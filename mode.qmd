...
```{r}
mode_first <- function(x, na.rm = FALSE, first_obvious = TRUE) {
  if (na.rm) {
    x <- x[!is.na(x)]
  }
  ux <- unique(x[!is.na(x)])
...
The `first_obvious` argument will rarely be relevant. It's discussed in *Implementation notes*.
...
mode_first <- function(x, na.rm = FALSE, first_obvious = TRUE) {
  ...
  if (any(is.na(x)) && is.na(adjudicate_mode_na(x, ux, mode1, TRUE))) {
    ...
    # *as a mode*. By default (`first_obvious = TRUE`),
    ...
    # Set `first_obvious` to `FALSE` to make the function
    # return `NA` in such cases. The idea is that, strictly
    # speaking, the true first mode is unknown here.
    # For example, `mode_first(c(2, 1, 1, NA))` is `1`
    # although `2` appears first. `2` might be another mode,
    # but the first value that is obvious as a mode is `1`.
    # `mode_first(c(2, 1, 1, NA), first_obvious = FALSE)`
    # returns `NA`.
  } else if (first_obvious) {
    mode1
  } else {
