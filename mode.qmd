...
mode_first <- function(x, na.rm = FALSE, first_known = TRUE) {
  ...
  if (any(is.na(x)) && is.na(adjudicate_mode_na(x, ux, mode1, TRUE))) {
...
The `first_known` argument will rarely be relevant. It decides whether the function accepts the first value known to be a mode (`TRUE`, the default), or whether it returns `NA` if a value might be the first-appearing mode but that's unknown due to `NA`s. See *Implementation notes*.
...
mode_first <- function(x, na.rm = FALSE, first_known = TRUE) {
  ...
  # the mode. If so, the present function returns the mode;
  # if not, it returns `NA`.
  if (any(is.na(x)) && is.na(adjudicate_mode_na(x, ux, mode1, TRUE))) {
    return(methods::as(NA, typeof(x)))
  # Although `mode_first()` is agnostic about other modes,
  # the mode it returns is the one that first appears
  # *as a mode*. By default (`first_known = TRUE`),
  # if a value might be a mode depending on true values
  # behind `NA`s but appears before the first `mode1`
  ...
  # although `2` appears first. `2` might be another mode,
  # but the first value that is known to be a mode is `1`.
  # `mode_first(c(2, 1, 1, NA), first_known = FALSE)`
  # returns `NA`.
  } else if (first_known) {
    return(mode1)
  }
