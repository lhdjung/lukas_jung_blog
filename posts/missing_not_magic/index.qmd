...
Some vectors have such a high share of `NA`s that the median can't be confined to a specific range. Even `median_range()` will return missing values, so we need a new trick.

```{r}
many_na <- c(7, 7, NA, NA, NA, NA)
median_range(many_na)
```

The new solution, `median_df()`, first attempts to find a non-`NA` median. If it can't, it ignores one missing value and tries again. If the result is still unknown, it will ignore one more `NA`; and so on until an estimate is found:

```{r}
median_df(list(many_na))
```
