...
You first need to collect all the names in a data frame: one column with the old names and one with the new ones. Besides a path argument, `rename_variables_df()` also takes that data frame (`df_names`) as well as the names of two columns that store the old and new names (`col_pattern` and `col_replacement`) in the data frame. Further options are passed down to `gsub()`, which takes care of the actual replacement.
...
# `col_replacement`:
rename_variables_df <- function(path, df_names, col_pattern, col_replacement,
